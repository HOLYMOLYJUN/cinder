...
**꺼져 있는 것이 기본값이다.** `js/config.js` 의 `NET.HOST` 가 비어 있으면 확성기는
아예 없는 기능처럼 군다 — 버튼도 안 보이고 `C` 도 먹지 않는다. `index.html` 을 더블클릭해
연 경우(`file://`)와 아티팩트 한 장으로 구운 경우에는 네트워크가 어차피 막혀 있으므로,
**꺼진 채로 게임이 멀쩡한 것이 정상 동작이지 예외가 아니다.** 그래서 빌드를 둘로 가르지 않았다.
