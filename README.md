...
날짜로 나뉘면 **매일 아침 모든 층이 텅 빈다.** 처음 올라온 사람이 보는 것은
"아직 아무도 없구나"가 아니라 "이 기능은 없는 건가"다 — 그 둘은 화면에서
구별되지 않는다. 그래서 층마다 한둘씩 탑이 스스로 남긴다(`js/guide.js`).

서버에 심지 않고 **클라이언트가 그날 지형에서 만든다.** 지형이 오늘 것으로 정해져
있으므로 같은 날 오르는 사람은 같은 자리에서 같은 말을 보고, 내일 지형이 바뀌면
길잡이도 알아서 따라간다. 서버에 심어 두었으면 내일 아침 전부 벽 속에 박혔을 것이다.
덤으로 **서버가 죽어 있어도 층이 비지 않는다.**

**하는 말은 전부 사실이다.** 계단 옆에서 「계단이 있다」라고 한다. 거짓을 심으면
사람이 남긴 말까지 못 믿게 되고, 그러면 흔적 전체가 죽는다.
...
- 탑이 남긴 길잡이 — 층마다 한둘, 그날 지형에서 만들어지고 하는 말은 전부 사실이다. 서버가 없어도 있다
