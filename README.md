...
| `Z` / `J` | 원거리 — **가장 가까운 것을 스스로 겨눈다.** 대각선도 된다. **마법사와 엘프만** 쓰고, 둘 다 첫 칸부터 쓴다. **지팡이를 들면 불덩이**가 나간다. 한 번 쏘면 한 턴(불덩이는 두 턴) 쉬어야 한다 |
...
`I` · `B` 로 가방을 열고, 칸을 두드려 끼거나 벗거나 내려놓는다. 상점에서는 숫자키로 구입.
...
| `assets/ui/` | 가방 창의 판 둘(장착 · 가방) | 마인크래프트풍 인벤토리 · **어울리는 게임이면 자유롭게 쓰라** · 갈색 계열로 바꿔서 굽는다 |
...
**원거리는 고른 사람이 정한다.** 마법사와 엘프만 쓰고, 기사·리자드·드워프에게는
아예 없는 조작이다 (`heroes.js` 의 `melee`).

예전에는 「던지던 손」이라는 기억을 되찾아야 열렸다. 그러면 같은 사람으로 시작해도
어떤 판은 원거리가 있고 어떤 판은 없어서 **「이 사람은 이렇게 싸운다」가 판마다
달라졌다** — 고르는 화면에서 한 줄로 약속해 놓고 그걸 주사위에 맡긴 셈이다.
3층 보장 지급으로 덧대 봤지만, 덧댈 것이 필요하다는 게 이미 답이었다.

- **마법사와 엘프는 첫 칸부터 쏜다.** 둘 다 손에 지팡이와 활을 들고 시작하므로
  1층에서부터 그 사람의 싸움이 난다. 무기를 검으로 바꿔도 조작은 남는다 —
  조작은 손에 든 것이 아니라 사람의 것이다.
- **기사·리자드·드워프는 붙어서만 싸운다.** `Z` 가 잠긴 게 아니라 없는 조작이다.
  리자드는 「붙어서 물고 물러서기」가 특성인데 멀리서 쓸 수 있으면 물 이유가 없고,
  드워프의 도끼는 「제일 아프지만 한 칸」이라는 거래 자체가 사라진다.
...
**장착한 무기는 캐릭터 손에 그려진다.** `Render.heldWeapon` 이 가방 아이콘을 그대로
...
가방 설명의 「무기」 자리에도 갈래와 하는 일을 적는다 — `검 · 앞과 그 양옆, 세 칸을 쓴다`.
...
「던지던 손」이라는 기억이 있었다. 되찾으면 원거리가 열리고, 운이 나쁠까 봐 3층에서
확정으로 줬다. 그 보장 장치가 이미 답이었다 — **덧대야 굴러가는 규칙은 틀린 규칙이다.**

지금은 사람이 정한다. 마법사와 엘프만 쏘고, 둘 다 첫 칸부터 손에 들고 시작한다.
그래서 고르는 화면의 한 줄(「처음부터 마법으로 싸운다」)이 1층에서 이미 참이다.

기억은 여덟 개가 됐다. 되짚기의 「던지던 손」 장(章)은 그대로 뒀다 — 붓을 던진 밤의
이야기지 조작이 아니고, `STORY` 는 되찾은 기억과 무관하게 전부 흐른다.
그래서 장과 기억이 1:1 이던 짝이 하나 끊어졌다. `test-story` 는 방향을 뒤집어
**「되찾는 기억은 전부 제 장이 있다」**만 잰다.

## 손가락에는 「공격」 하나

예전 모바일 버튼은 「원거리」였다. 근접 사람에게는 아예 안 보이는 버튼이었고,
그러니 **폰에서 근접 공격은 오직 몸으로 밀고 들어가는 것뿐**이었다.
검이 앞과 양옆 세 칸을 친다는 규칙이 있는데 그걸 쓸 버튼이 없었다.

이제 누가 누르든 「공격」 하나고, 무엇이 나가는지는 그 사람이 정한다 —
원거리 사람은 쏘고, 근접 사람은 **닿는 곳을 스스로 찾아** 친다.
원거리가 이미 스스로 겨누고 있었으므로(버튼 한 번 = 알아서 조준) 같은 규칙을 따랐다.
두 버튼이 다르게 굴면 그것만으로 손이 헛돈다.

잠기는 조건도 갈린다 — 원거리는 쉬는 동안(`rangedCd`), 근접은 닿는 것이 없을 때.

### 휘두른 자국

근접은 지금까지 몸이 살짝 튀어나갔다 오는 것이 전부였다. 그러면 **무기가 어디까지
닿았는지가 안 보인다** — 검은 세 칸, 창은 두 칸을 치는데 화면에는 숨은 규칙이었다.

친 칸마다 호를 하나씩 긋는다. 깊게 친 칸은 진하고 쓸린 칸은 연하다 — 배율이 그대로
보이므로 「정면이 제일 아프다」를 말로 안 적어도 된다.
**맞은 칸이 아니라 닿은 칸 전부**에 긋는다. 맞은 데만 그으면 옆칸이 비었을 때
한 칸짜리 무기로 보인다.

## 모닥불도 지나갈 수 있다

「불 앞에서 아무것도 안 하고 지나갈 수 있으면 그건 선택이 아니라 무시해도 되는 창이
된다」 — 그래서 나가는 길을 안 뒀었다.

그런데 「불을 쬐지 않고」 업적을 노리는 사람에게는 그 규칙이 벽이었다. 모르고 한 칸
잘못 밟으면 그 판의 업적이 그 자리에서 끝난다. **되돌릴 방법이 없는 실수는 판단이
아니라 사고다.**

「그냥 지나간다」를 넷째로 뒀다. 불은 그대로 남으므로 지나가는 것이 포기가 아니라
미루는 것이 된다. 창 안에 나가는 길이 생겼으니 Esc·뒤로가기로도 나간다.

## 판을 두고 나가는 문

폰에는 Esc 가 없다. 그래서 **판을 멈추고 첫 화면으로 돌아갈 길이 아예 없었다** —
안드로이드 뒤로가기는 앱을 끄는 것이지 판에서 나오는 것이 아니다.

도감 맨 아래에 「탑에서 내려간다」를 둔다. 도감은 폰에서는 버튼으로,
키보드에서는 Esc 로 열리므로 **한 문으로 둘 다 된다.**

`backToTitle` 과 반대다. 저쪽은 끝난 판을 **지우고** 나가고(끝난 판이 이어하기로
남아 있으면 첫 화면에 「15층부터 이어서」가 뜼다), 이쪽(`leaveToTitle`)은 하던 판을
**두고** 나간다 — clearRun 을 안 부르고 오히려 한 번 더 저장한다.

타이틀에서 도감을 열었을 때는 안 보인다. 이미 나와 있는 사람에게 「나가기」를
내뮸는 셈이다. 탭과 반대쪽 끝에 두는 것도 같은 이유다 — 읽으러 열었다가
실수로 누를 자리가 아니다.

## 되짚기 건너뛰기는 언제나 보인다

한 번 끝까지 본 사람에게만 내밀었었다. 처음 보는 사람에게 내밀면 「읽을 만한 것」이
아니라 「넘겨도 되는 것」으로 먼저 읽힌다는 이유였다.

그럴듯했지만 값이 컸다. 처음 온 사람이 이 이야기에 관심이 없으면 끝날 때까지 갇힌 채
기다려야 하고, 그건 이야기를 아끼는 것이 아니라 **사람을 붙잡아 두는 것**이다.
...
## 처음 켠 사람에게는 튜토리얼을 만들지 않는다

저장소를 비우고 실제로 켜 보면 이렇다 — **로그가 텅 빈 채로 시작한다.**
방향키·물약·불씨·도감 버튼이 있는데 무엇인지 아무도 말해 주지 않고,
무엇보다 **목표를 모른다.** 로그라이크를 안 해 본 사람은 「계단을 찾아
올라간다」를 모르고, 계단은 시야 밖이라 화면에 보이지도 않는다.

그렇다고 튜토리얼 화면을 만들지 않는다. 이 게임은 이미 제 화법이 있다 —

> `벽이 손에 닿습니다. 한 번 더 밀면 여기에 남길 수 있습니다.`

화살표도 손가락 그림도 없이 흔적 기능을 이렇게 가르치고 있다.
**상황이 왔을 때, 로그에, 한 줄.** 조작도 같은 목소리로 가르치는 것이 맞다.

| 언제 | 무엇 |
|---|---|
| 판을 시작할 때 | 계단을 찾아 오르십시오. 열다섯 층입니다. |
| 몬스터를 처음 볼 때 | 부딪히면 칩니다. |
| 처음 다칠 때 (60% 밑) | 다쳤습니다. 물약은 아래 「물약」. |
| 계단을 처음 볼 때 | 계단입니다. 밟으면 오릅니다. |

**넉 줄뿐이고, 첫 판에만 나온다**(`runs` 가 0인 사람). 아는 사람에게 두 번째
판부터 같은 말을 하면 그건 안내가 아니라 잔소리고, 건너뛰기 단추를 달아야
하는 종류의 것이 된다. 튜토리얼은 그래서 건너뛰기가 필요하고, 이 방식은 안 그렇다.

> 더 넣고 싶은 줄이 생기면 그건 대개 **화면이 스스로 말하지 못하고 있다는
> 신호**다. 문장을 늘리기 전에 화면을 고치는 쪽을 먼저 본다.

**못 하는 것에는 이유를 말한다.** 「불씨」 버튼은 기억을 되찾기 전에는 눌러도
조용히 아무 일도 안 했다 — 버튼은 화면에 있으므로 그건 고장으로 읽힌다.
이유를 말해 주면 그때부터 **아직 못 여는 것**이 된다.
...
모바일 「공격」 버튼이 닿는 것이 없을 때 흐려지는 것과 같은 규칙이다 — 누를 수 없는 순간을 미리 보여 준다.
...
전에는 장비를 밟는 순간 비교창이 떴다. 그 자리에서 「바꿀까 말까」를 물었고
두고 가면 끝이었다. 그러면 「지금은 안 쓰지만 이따가 쓸모가 있는 것」을 들고
다닐 수가 없어서, 모든 고르기가 밟는 그 자리에서 되돌릴 수 없게 된다.

가방은 그걸 미룬다. 버리는 것은 여전히 되돌릴 수 없지만 **고르는 때**는
언제든 다시 온다. 열두 칸이고(`BAG_MAX`), 차면 줍지 못한다 — 대신 바닥에
그대로 남으므로 비우고 다시 와서 가져가면 된다.

끼면 끼고 있던 것이 가방으로 돌아온다. 가방이 꽉 차 있으면 **빼낸 그 자리에**
들어간다. 가방이 찼다고 갈아끼기까지 막히면 그건 설명할 수 없는 벽이다.

### 창은 두 줄이다

위줄은 두 칸 — 왼쪽은 입고 든 것, 오른쪽은 고른 물건의 값. 아랫줄은 가방
6x2 를 넓게 깔았다. 설명이 **옆에** 있어야 「지금 낀 것 ↔ 고른 것」이 한눈에
들어오고, 가방이 넓어야 칸 하나가 손가락만해진다 (320px 폰에서 39px).

판은 CSS 격자가 아니라 **도트 그림**이다. 격자로 그리면 상인·대장간과 같은
네모가 되는데, 가방은 「물건을 담는 것」이라 판이 훨씬 그럴듯하다.
`tools/make-inventory.js` 가 원본 인벤토리를 갈색으로 갈아입혀 두 장으로 굽고
**그림과 칸 좌표를 한 곳에서** 내놓는다(`js/bagslots.js`) — 둘이 갈라지면
아이콘이 칸 밖에 뜬다.

### 사이에 서 있는 모습

입은 칸과 든 칸 사이가 비어 있어서 거기에 지금 모습을 세운다. 던전에서
그리는 것과 **같은 함수**로 그린다 — `Render.portrait` 이 `sprite` 와
`heldWeapon` 을 그대로 불러 쓴다. 따로 그리면 각도나 키가 조금씩 어긋나고,
그것만으로 「내가 든 것」이 아니라 「비슷하게 생긴 그림」이 된다.

크기는 **칸을 기준으로** 잡는다. 빈 자리에 맞춰 꽉 채우면 도트가 너무 굵어져
사람이 아니라 색 덩어리로 보인다 — 뜻밖에 이것이 가장 컸다.

### 창은 한 얼굴이어야 한다

같은 부류의 자리는 같은 틀을 쓴다. 어겼던 곳이 둘 있었다.

**대장장이**는 모닥불 창을 빌려 썼다. 상인과 대장장이는 둘 다 골드를 쓰는 자리인데
창 생김새가 딴판이라 폰에서는 글자 크기까지 달랐다. 상인의 틀로 옮겼다.

**모닥불·동행·쪽지**는 결말 화면을 빌려 썼다. 결말은 판을 끝내는 한 번뿐인 화면이라
제목이 34px 에 가운데 정렬인데, 이 셋은 판마다 여러 번 뜨는 **고르는 창**이다.
그 무게가 매번 과했고, 같은 층에서 밟는 상인과 나란히 놓고 보면 두 창이 다른 게임에서
온 것처럼 보였다. 역시 상인의 틀로 옮겼다 — **배경의 불빛만 남긴다.**
그건 틀이 아니라 자리의 표시다.

### 모서리 색은 판에서 떠 온다

가방 판은 도트 그림이라 위·왼쪽이 밝고 아래·오른쪽이 어두워 **튀어나와 보인다.**
그 옆에 놓인 설명 칸은 CSS 로 그린 1px 네모라 평평했다. 한 창 안에서 두 재료가
섞이면 판만 그림처럼 보이고 나머지는 얹어 놓은 것처럼 보인다.

값을 짐작하지 않는다. 구운 판(`assets/ui/equip.png`)의 화소를 떠서 적어 뒀다:

```
--edge-hi: #3A2F25   /* 위·왼쪽 — 판의 밝은 모서리 */
--edge-lo: #0C0A08   /* 아래·오른쪽 — 판의 어두운 모서리 */
```

이 둘을 창 식구 전부에 입힌다 — 상인·대장장이·모닥불의 판과 줄, 가방의 설명 칸과
행동 버튼, 캐릭터 카드, 터치 버튼, 그리고 모든 유령 버튼.

예전에도 음영이 있는 곳은 있었다(`.hero`, `.touch-acts button`). 다만 밝은 쪽에
`--line` 을, 어두운 쪽에 `#100D0A` 를 **어림잡아** 썼다. 판보다 한 단계씩 어긋나
있어서, 나란히 놓으면 같은 재료로 안 읽혔다. 이제 `#100D0A` 는 한 곳도 없다.

큰 창에는 검은 줄(`box-shadow: 0 0 0 1px #000`)을 하나 더 두른다. 판도 바깥이
검은 윤곽이라, 그게 있어야 「닫힌 물건」으로 보인다.

### 캐릭터 선택에서 숫자를 뺐다

예전에는 카드마다 `체50 공6 방3 속10` 을 깔고 아래에 한 번 더 풀어 적었다.
「별점보다 숫자가 정직하다」는 이유였는데, **고르는 자리에서 그 숫자로 할 수 있는 일이
없었다.** 첫 판에는 비교할 기준이 없고(공격 6이 센 건지 약한 건지 모른다), 여러 번
올라 본 뒤에는 이미 안다. 그 사이에서 숫자는 「이 게임은 표를 읽는 게임」이라고 먼저
말해 버렸다.

무엇을 들고 어떻게 싸우는가는 한 줄 설명이 말한다 — 그게 실제로 고르는 근거다.
좁은 화면에서 카드의 숫자를 감추고 아래로 내리던 미디어 질의 하나도 같이 사라졌다.

### 장비 줄을 없애고 「가방」 버튼을 뒀다

던전 화면 위에는 낀 장비 다섯과 능력치를 한 줄로 깔아 두었다. 가방 창이 생기면서
그 전부가 그 안에 있게 되었고, **같은 것을 두 자리에 두면 한 자리는 늘 낡는다.**

줄을 없애고 그 자리에 여는 버튼만 남긴다. 터치 기기에는 아래 행동 버튼에 이미
「가방」이 있어 이쪽은 감춘다 — 웹에는 `I`·`B` 밖에 없어서 그것만으로는 있는 줄을
모른다.

그 줄이 혼자 말하던 것(능력치 · 갖춰 입기 몇 조각 · 동행)은 **가방을 열고 아무것도
안 골랐을 때** 설명 칸이 대신 말한다. 빈 칸으로 두면 아까운 자리였다.

### 두 판의 아랫줄을 맞추는 법

가방 창의 위줄은 장착판(그림, 비율 고정)과 설명(글, 길이 제각각) 두 칸이다.
그대로 두면 짧은 쪽 밑이 통째로 빈다.

**줄 높이를 장착판이 정하게** 하고, 설명은 안칸(`.bag-detail-in`)을 절대 위치로
띄워 그 안에서 구르게 했다. 글 길이가 줄을 못 밀므로 아랫줄이 항상 맞는다.
장착판의 세로 비율(92x101)은 그래서 칸 수가 아니라 **옆 칸이 정한 숫자**다.

### 가방을 넣으면서 흘린 것 셋

비교창을 없앨 때 그 창이 **혼자 하던 일**이 같이 사라졌다. 테스터가 셋 다 찾아 줬다.

**정체불명을 열 길이 없었다.** 예전에는 「교체」를 고르는 순간 `revealGear` 가 돌았다.
가방에는 그 자리가 없었고, `bagEquip` 은 아예 `if (g.unknown) return null` 로 막고
있었다 — 주우면 가방에서 `?` 로 앉아 버리는 것 말고 할 수 있는 일이 없었다.

이제 **끼는 것이 곧 여는 것**이다. `bagEquip` 안에서 드러난다 — 둘을 갈라 두면
다음에 또 한쪽만 남는다. 버튼 글자도 「열어 본다」로 바꿔서 무슨 일이 벌어지는지
먼저 말한다.

**버리면 정체가 드러났다.** `bagDrop` 이 `seen: true` 를 달아서, 안 열어 본 물건도
바닥에 제 그림이 깔렸다. 「버렸다 줍는다」가 공짜 감정이 되어 도박이 통째로 사라진다.
`seen: !g.unknown` 으로 고치고, **그리는 쪽에서도** 한 번 더 막았다 — 정체를 흘리는
자리는 한 곳만 막으면 다음에 또 샌다.

**상점에서 산 것이 낀 것을 덮어썼다.** 사면 그 자리에서 입히고 끼고 있던 것을
버렸다. 가방이 없던 시절의 규칙인데, 가방이 생긴 뒤로는 그게 **돈 내고 내 장비를
버리는 일**이다. 산 것도 바닥에서 줍는 것과 같은 자리로 간다. 가방이 차 있으면
**돈을 받기 전에** 막는다.

셋 다 한 문장으로 줄면 이렇다 — **얻는 것은 전부 가방을 거친다.** 예외를 두면
그 예외가 낡는다.

### 정체불명 — 갈아끼기에 판단을 돌려준다

갈아끼기가 "숫자가 크면 먹는다"로만 끝나면 그건 판단이 아니라 산수다.
...
테스터가 그대로 적어 보냈다 — **「뭔 보스가 플레이어 한대도 못때리노」.**

기술이 둘뿐이었고 둘 다 한 걸음이면 피해졌다. `slam` 은 보스 주변 여덟 칸이라
멀어지면 끝이고, `curse` 는 **지금 서 있는 칸 하나**라 아무 데나 한 칸 가면 끝이다.
예고를 보고 한 번 움직이면 되니 피하는 것이 **판단이 아니라 절차**였다.

머리말에 적힌 규칙은 틀리지 않았다 — 「피할 방법이 없는 기술은 설계 실패」.
문제는 반대쪽 끝이었다. **네 방향이 전부 열려 있었다.**

### 고를 것을 줄인다

기술 셋을 더했다. 전부 여전히 피해지되, 고를 것이 줄어든다.

| | 하는 일 | 남는 길 |
|---|---|---|
| `sweep` | 보스와 나를 잇는 결을 따라 세 칸 | **둘** — 옆으로만 |
| `pin` | 내 자리 + **물러날 칸** | 셋 — 도망치는 쪽이 막힌다 |
| `lunge` | 결을 친 뒤 **그 끝으로 옮겨 온다** | 넷 — 다만 피해도 눈앞에 선다 |

`lunge` 가 제일 큰 조각이다. 보스가 한 대도 못 때린 진짜 이유는 기술이 약해서가
아니라 **거리가 안 좁혀져서**였다. 속도가 비슷하니 물러서면 영원히 물러설 수 있었다.

그리고 **피했다고 보스가 가만히 서 있지 않는다** — 한 걸음 다가온다.
때리지는 않는다. 피한 것은 피한 것이다.

### 기술을 늘렸더니 보스가 약해졌다

기술 한 번은 예고 한 턴과 터지는 한 턴을 쓴다. 그 두 턴은 **평타를 안 치는 턴**이라
피해 버리면 통째로 공짜다. 간격을 3→2 로 줄이고 기술을 더했더니 봇 기준
클리어율이 25 → **32%** 로 올라갔다. 만들려던 것과 정반대다.

간격을 3 으로 되돌리고 그릇을 키웠다(66→82 · 104→126 · 190→230). 28% 로 돌아왔다 —
원래 25% 와 오차(±3pp) 안이다. **피하는 것으로 벌지 않는 값은 그릇뿐이다.**

### 이건 시뮬로 못 잰다

`tools/sim.js` 의 봇은 `markedTiles()` 를 그대로 읽어 예고를 **100% 피한다.**
그러니 어떤 기술을 더하든 봇에게는 공짜 턴이 늘 뿐이고, 여기서 나오는 숫자는
「보스가 얼마나 무서운가」가 아니라 **「생각 없이 약하게 만들지는 않았는가」**일 뿐이다.

패턴이 실제로 무서운지는 **사람이 해 봐야 안다.**

### 안식처가 너무 많은 것 같다 — 두 번 재고 두 번 다 되돌렸다

쉼터가 3·6·9·12 로 넷이다. 열다섯 층 중 넷이 쉼터고 넷이 보스라, **그냥 오르는
층은 일곱뿐**이다. 많아 보이는 것이 맞다.

그런데 그것보다 눈에 걸린 것은 **자리**였다. 200판 표의 「진입 체력」 칸을 보면:

| 보스 | 직전 쉼터 | 진입 체력 |
|---|---|---|
| 5층 문지기 | 3층 (두 층 전) | 91% |
| 10층 이름을 가진 것 | **9층 (바로 전)** | **98%** |
| 13층 용 | **12층 (바로 전)** | **97%** |
| 15층 등불지기 | 12층 (세 층 전) | 55% |

쉼터 바로 뒤의 보스는 **만피로 시작한다.** 그리고 실제로 사람을 죽이는 것은
15층 하나다 — 처음 오르는 사람 기준 80판이 거기서 끝난다. **소모를 겪고 만나는
보스만 보스 노릇을 하고 있었다.**

그래서 「개수가 아니라 자리가 문제」라고 보고 두 번 재 봤다.

| | `[3,6,9,12]` 지금 | `[3,7,11]` 셋으로 | `[3,6,8,11]` 넷인 채 앞으로 |
|---|---|---|---|
| 클리어 (기사·기억0) | **27.5%** | 19.5% | 16.0% |
| 15층 도달 | 40% | 30% | 33% |
| 10층 보스 통과 | 73% | 63% | 68% |
| 클리어 (기사·기억8) | 60.0% | 58.0% | 53.5% |

**둘 다 나빠졌다. 개수를 유지한 쪽이 더 나빴다.**

(세 번 다 **기사**로 쟀다. `tools/sim.js` 는 사람을 안 주면 기사로 도므로
셋이 같은 조건이다. 다른 사람은 값이 다르지만 비교는 성립한다.)

이유가 표 안에 있었다. `[3,6,8,11]` 에서 **8층 쉼터에는 82%로 도착한다.**
성한 몸으로 불을 쬐면 그 쉼터는 없는 것과 같다. 쉼터를 앞으로 당기면 개수는
그대로여도 **실제로 회복되는 양은 줄어든다** — 뒤로 갈수록 다치기 때문이다.

그러니 「보스를 만피로 만난다」와 「쉼터가 뒤쪽에 있다」는 **같은 사실의 두 얼굴**이라,
하나만 떼어낼 수 없었다. 자리만 고치는 방법은 없다.

그리고 첫 번째(`[3,7,11]`)에서 더 중요한 것이 보였다 — **손해가 한쪽에만 떨어진다.**
아직 아무것도 못 되찾은 쪽은 8pp 를 잃고(오차 ±3pp 밖), 다 되찾은 사람은 2pp 로 오차 안이다.
쉼터는 **아직 아무것도 못 가진 사람이 기대는 것**이라, 줄이면 그 사람만 맞는다.

### 진짜 모양은 이쪽이었다

재면서 나온 숫자 중 제일 큰 것:

> 기사·기억0 으로 **1~10층에서 죽는 것은 66판, 11~15층에서 죽는 것은 129판.**
> 15층 하나에서만 80판.

탑은 10층까지 거의 안 죽고 11층부터 벽이다. **쉼터는 이 모양의 원인이 아니라
이 모양 때문에 필요한 것**이다 — 빼면 벽이 더 가팔라질 뿐, 앞쪽이 재밌어지지 않는다.

「너무 많다」는 느낌의 진짜 원인은 개수가 아니라 **1~10층이 너무 안 죽는다**는 것이고,
손댈 곳은 거기다. 마침 사람 다섯을 4·6·8·10·11층에 세운 참이라, 그 효과부터
**사람이 해 보고** 판단하는 것이 순서다. 쉼터는 `[3, 6, 9, 12]` 그대로 둔다.

## 용은 복도에서 마주치는 것이 아니다

테스터가 용을 보고 적어 보냈다 — **「개 나약함」.**

맞는 말이었다. 용은 12층부터 그냥 나오는 보통 몬스터였다. 다른 것들 사이에 섞여
걸어다니다 두어 대 맞고 죽었다. **복도에서 지나쳐지는 것이면 그건 용이 아니다.**

몬스터 목록에서 빼서 **13층의 주인**으로 올렸다. 몸집은 두 배(`scale: 2`)로 그리고,
숨결(`breath`)은 결을 따라 여섯 칸을 태우며 **맞으면 옷에 불이 옮는다** — 피한 뒤에도
몇 걸음은 계속 탄다. 지금까지 옷에 불을 붙이는 것은 이 하나뿐이다.

### 왜 13층인가

쉼터가 3·6·9·12 이고 보스가 5·10·15 다. 12층이 쉼터라 용을 거기 두면
**마지막 쉼터가 사라진다.** 13에 두면 12에서 쉬고 → 13에서 용 → 14에서 한 숨 →
15에서 끝이 된다. 오르는 결이 그대로 남는다.

200판으로 재 보니 13층에 닿은 118판 중 **27판이 거기서 끝났다**(23%).
지나쳐지지는 않는다.

## 여섯 번째 사람 — 다섯을 다 올라야 열린다

다섯의 축을 늘어놓고 보니 하나가 걸렸다.
...
**전부 전투 수치다.** 다섯을 다 오른 사람에게 여섯 번째로 줄 것이 또 전투 수치면
그건 여섯 번째 근접이지 새로운 사람이 아니다.

**금**을 축으로 잡았다. 금은 이미 상점과 대장간이 쓰고 있는데, 정작 버는 방법은
잡는 것 하나뿐이라 한 번도 축으로 선 적이 없었다.

그림은 NPC 팩의 도둑이다 — 탑 안에 서 있던 것이 밖으로 나와 고를 수 있게 된
첫 번째 사람이라, 사람 다섯을 몬스터로 세운 것과 한 짝이다.

### 훔치기 — 잡을 때가 아니라 때릴 때

때린 것에게서 금이 딸려 온다. **한 마리에서 한 번만.**

한 번만인 것이 규칙의 전부다. 같은 것을 계속 패도 더 안 나오므로, 여럿이 몰려
있으면 **한 대씩 돌아가며 치는 것**이 이득이 된다. 「하나를 끝까지 잡는다」가
언제나 옳던 판에 처음으로 다른 답이 생긴다. 그릇이 제일 얇은 사람이 무리 속을
도는 것이라 공짜는 아니다.
...
「단검도 주고 표창도 던지게」가 요구였는데, 그대로 하면 **엘프가 된다.**

엘프는 활이 무기 칸을 차지해서 붙는 것과 던지는 것이 같은 무기다. 도둑은 단검을
쥔 채로 따로 던지므로 둘이 갈려야 한다 — 안 그러면 단검을 벼릴수록 표창도 같이
세져서 그냥 「단검을 든 엘프」다.

그래서 표창의 위력에서 **무기 칸의 공격을 통째로 뺀다.** 벼린 값도 `g.mod` 에
직접 얹히므로(`forgeGear`) 그만큼도 같이 빠진다 — 뺄 곳이 한 곳이라 어긋날 데가 없다.

대신 깎지 않는다. 던지기 0.7배·활 0.85배인데 **표창은 1.0배**다. 무기를 안 얹는
대신 값을 안 깎는 거래라, 초반에는 남들만큼 나가고 무기가 좋아질수록 뒤처진다.
그러면 손이 이렇게 간다 — **표창으로 떼어 놓고 단검으로 끝낸다.** 던지기가
주력이 되지 않는다.

`tools/test-thief.js` 가 이걸 지킨다. 같은 자리에 같은 것을 세워 두고 낡은 단검과
불씨 단검으로 400번씩 던져 합을 비교한다 — 근접 공격은 6에서 10으로 오르는데
표창의 합은 1595 대 1602 로 움직이지 않는다.

### 자기가 자기 조건이 되는 버그

여는 조건은 업적 「모두의 탑」(다섯 사람 모두로 탑을 오른다)을 그대로 쓴다.
그런데 그 업적을 세는 줄이 이랬다:
...
여섯 번째를 `HEROES` 에 넣는 순간 **여섯을 다 올라야 열리는 업적**이 되는데,
그 여섯 번째는 이 업적으로 열린다. 영원히 안 열린다. `BASE_HEROES`(hidden 이
아닌 사람들)를 따로 두고 그걸 센다.

### 잠금은 세 곳에서 같은 답을 해야 한다

「고르는 자리에 안 보인다」로 끝내면 저장에는 도둑이 들어가고 판에는 기사가 서는
상태가 만들어진다 — 고쳐지는 대신 **아무도 모르게** 어긋난다. 세 곳을 다 막는다:

| | 하는 일 |
|---|---|
| `pickableHeroes()` | 고르는 자리에 안 내놓는다 |
| `chooseHero()` | 저장에 안 넣는다 (`false` 를 돌려준다) |
| `currentHero()` | 저장에 박혀 있어도 안 돌려준다 |

세 번째가 실제로 필요했다. 남의 저장을 옮겨 오거나 손으로 고친 경우가 있다.

그리고 검사도 열고 재야 했다. `test-classes` 가 `chooseHero('thief')` 를 부르고
드랍을 쟀는데, 안 받아 주니 **기사의 숫자를 재 놓고 도둑이라고 적었다** —
단검이 2% 로 찍혔다. 시뮬(`tools/sim.js`)도 같은 함정이 있어 같이 고쳤다.

### 히든은 마지막까지 히든이다

업적 설명에 「도둑이 열린다」고 적으면 잠긴 채로도 읽혀서 히든이 아니게 된다.
`reveal` 을 따로 뒀다 — **열고 나서야** 보이는 줄이다.
무엇을 향해 가는지(`desc`)는 처음부터 보이고, 무엇이 나오는지는 연 뒤에 보인다.

### 숫자

처음 세운 값(공격 4 · 그릇 32)으로 200판을 돌렸더니 **7.5%** 였다 (기사 27.5%).
이유가 표에 그대로 있었다 — **5층에서 62판이 끝났고 문지기 통과가 68%** 였다
(기사 94%). 문지기 방어가 4인데 도둑의 손이 6이라 한 대에 2씩 들어갔다.
82짜리 그릇을 41번 쳐야 한다. 얇은 것이 아니라 못 이기는 것이었다.

공격 4→6 · 그릇 32→38 · 성장 0.52→0.58 로 올리고 다시 200판:
...
처음 오르는 쪽은 기사와 사실상 같다(오차 ±3pp 안). 기억을 다 되찾은 쪽만
8.5pp 높은데, **다섯을 다 오른 사람에게 주는 것**이라 그대로 둔다.
그쪽으로 더 기우는 이유는 아마 기본기가 낮아 되찾은 값이 비율로 크게 붙기
때문인데, 재서 확인한 것이 아니므로 여기까지만 적는다.

## 탑을 오르다 돌아오지 못한 사람들

NPC 팩에 스물여섯 종이 들어 있는데 **둘(대장장이·아이)만 쓰고** 나머지가 놀고 있었다.

이 게임은 먼저 오른 사람이 벽에 말을 남기는 게임이다. 그런데 탑에 있는 것은 전부
짐승이었다 — 말을 남긴 사람들이 어디로 갔는지가 **글로만 있고 그림으로는 없었다.**
쪽지 옆에 실제로 서 있어야 그 말이 무거워진다.

다섯을 세웠다. 숫자는 같은 층대의 짐승과 나란히 잡되 **성격을 다르게** 준다 —
같은 층에 체력만 다른 것이 하나 더 있는 것은 몬스터가 는 것이 아니다.

| | 층 | 하는 일 |
|---|---|---|
| 무너진 기사 | 4 | 첫 「사람」. 짐승보다 단단하고 짐승만큼 빠르다 |
| 재의 사냥꾼 | 6 | 활을 쏜다 — **주문이 아니다** |
| 처형인 | 8 | 느리고 물렁한데 한 방이 제일 아프다 |
| 무거운 기사 | 10 | 방어 10. 물리로는 잘 안 들어간다 |
| 도살자 | 11 | 체력과 공격이 둘 다 높고 방어가 낮다 |

### 활은 멀리서 오지만 주문이 아니다

여태 멀리서 때리는 것은 **전부 주문쟁이**였다. 판정 규칙이 `주문 > 공격 이면 원거리`
하나라서, 원거리형이 되려면 주문 수치를 올리는 수밖에 없었다. 그러니 마방 하나만
올려 두면 탑의 원거리 전부가 같이 막혔다 — **한쪽만 챙기면 되는 것은 선택이 아니다.**

`bow` 를 하나 뒀다. 활은 사거리는 주문과 같게 굴리되 **판정은 물리**라 방어로 막힌다.
수치도 그에 맞춰 공격 9 · 주문 0 으로 잡았다 — 예외를 판정 규칙에 넣지 않고
**수치를 규칙에 맞추는 쪽**으로 풀었다. 그래서 `rollDamage` 는 한 줄도 안 바뀌었다.

바뀐 것은 말투다. 「주문을 준비합니다」가 아니라 「시위를 당깁니다」,
「주문이 적중해」가 아니라 「화살이 박혀」. 도감에도 `원거리 · 활` 로 따로 적힌다 —
표에서 **공격 칸에 불이 들어와 있으므로** 무엇으로 막는지는 이미 보인다.

그릇은 16으로 얇게 뒀다. 붙으면 죽는 것, 그것이 사냥꾼이다.

## 궁수 셋은 제일 쉬운데 제일 컸다

셋 맞추기의 값을 다시 보다가 하나가 어긋나 있었다.

궁수 셋(긴 활 · 가죽 갑옷 · 날랜 장화)은 **평범한 등급이 섞여 있어 제일 일찍 맞춰진다.**
그런데 두 조각에 속도 +2, 다 맞추면 속도 +5 · 공격 +3 이었다. 속도는 이 게임에서
**한 턴의 행동 횟수**라 값이 제일 무거운 수치고, 이 셋을 입는 사람은 이미 제일 빠른
엘프(13)다. **제일 쉬운 것에 제일 센 값**이 붙어 있으면 고를 일이 다시 없어진다.

성격은 그대로 뒀다 — 궁수 셋은 빠른 셋이다. 대신 두 조각짜리를 반으로 줄이고
큰 몫을 **세 번째 조각으로 옮겼다.** 셋 맞추기의 재미는 마지막 하나에 있다.

| | 전 | 후 |
|---|---|---|
| 두 조각 | 속도 +2 | 속도 +1 |
| 다 맞춤 | 속도 +5 · 공격 +3 | 속도 +3 · 공격 +3 |

다 맞춘 값이 창병 셋(공격 +5 · 속도 +1)과 같은 크기가 됐다. 둘 다 일찍 맞춰지는 셋이니
크기가 같아야 하고, **다른 것은 성격이지 값이 아니다.**

## 갖춰 입기 — 갈아끼기에 처음 생긴 「그렇지만」
...
### 원거리를 사람에게 묶고 나서 — 다시 재고 다시 잡았다

원거리를 마법사·엘프에게만 남기면서 **리자드와 드워프에게서 조작 하나를 빼고
아무것도 안 돌려줬다.** 200판으로 재 보니 그 값이 그대로 나왔다.
...
가장 센 쪽과 가장 약한 쪽이 **7.4배에서 2.0배**로 좁혀졌다. 순서는 그대로다 —
마법사가 입문, 기사가 표준, 리자드·드워프가 도전.

### 세 번 헛짚었다

**사거리를 줄여 봤다 (8 → 7).** 클리어율이 소수점도 안 움직였다.
이유는 이미 같은 파일에 적혀 있었다 — **겨누려면 보여야 하고 불씨 반경은 7이다.**
8은 처음부터 닿은 적 없는 숫자였다. 줄여도 같은 값을 다시 쓴 셈이다.

**위력을 깎아 봤다** (화살 0.85→0.78, 불덩이 0.8→0.72). 이것도 안 움직였다.

**지팡이 수치를 깎았다** (꼭대기 18 → 13). 66.5 → 61.5%, 5pp 뿐이었다.
이건 진짜 문제이긴 했다 — 검이 4·6·8·10, 도끼가 5·7·11·14인데 지팡이만
4·9·12·18 이었고 몬스터의 마방(평균 2.9)과 방어(2.8)는 같아서 **그 두 배를
막아 주는 것이 없었다.** 다만 주범은 아니었다.

### 주범은 **간격**이었다

수치를 다시 보니 답이 있었다. 원거리 둘은 한 판에 43~44마리를 잡고 근접 셋은
23~31마리였다. **한 방이 센 것이 아니라 다가오는 동안 공짜로 때리는 턴이 쌓인 것**이다.
그러면 손댈 곳은 위력이 아니라 **얼마나 자주 쏘느냐**다.

재사용 간격을 2·3 → 3·4 로 조였다. 엘프 57.5 → 42.5%, 마법사 61.5 → 46.5%.
한 번에 15pp 씩 움직였다 — 앞의 셋을 다 합친 것보다 크다.

### 근접 셋에게는 뺀 값을 돌려줬다

**리자드** — 손에 든 공격이 기사의 절반(5 대 10)이고 성장도 제일 느렸다(0.40).
독이 시간으로 갚아 준다지만 **그 시간까지 버틸 힘이 없었다.** 공격 3→5, 성장
0.40→0.55. 9 → 23%. 독은 안 건드렸다 — 모자랐던 것은 「독이 들 때까지 서 있는 힘」이었다.

**드워프** — 숫자에 모순이 있었다. 속도가 7로 기사(10)보다 한참 느린데 **방어도
그릇도 기사보다 작았다**(2/46 대 3/50). 느린 사람은 같은 시간에 더 맞는데 거기서
더 약하면 그건 특성이 아니라 그냥 못한 사람이다. 방어 2→5, 그릇 46→54,
마방 0→1. 13 → 28%. 속도 7은 그대로 뒀다 — 빠르게 만드는 순간 기사와 같아진다.

### 기사는 못 움직였다

성장을 0.70→0.82 로 올려 봤다. 10층 주인 통과는 64→74% 로 올랐는데 **클리어율은
25→23% 로 안 움직였다**(200판 오차 ±3pp). 병목이 옮겨갔을 뿐 없어지지는 않았다.
근거가 없는 숫자는 안 남기므로 되돌렸다. **기사가 왜 거기서 막히는지는 아직 모른다.**

### 지팡이에는 바닥이 있다

나무 지팡이를 6으로 내렸더니 `test-levels` 가 잡았다 — **기사의 기본 공격이 6이라
같아지는 순간 물리 판정이 유지된다.** 들어도 마법사가 안 되는 지팡이는 주울 이유가
없는 함정 아이템이다. 7로 올렸다.

성장(`grow.pow`)은 공격과 주문에 같이 붙으므로 차이는 레벨이 올라도 그대로다 —
1레벨에서 넘기면 끝까지 넘고, 같으면 끝까지 막힌다.
...
넷을 더했다. 「안 했다」 쪽 둘 — **두드리지 않고**(대장장이에게 한 번도 안 맡김),
**맨정신으로**(물약 없이). 둘 다 한 판 안에서 끝난다.

나머지 둘은 **판을 넘어 쌓이는 첫 업적**이다 — **고개를 끄덕여**(남의 말에 열 번),
**벽에 남기는 손**(열 번 긁어 둠). 흔적은 애초에 「여러 번 올라온 사람」을 전제로 하는
기능이라 한 판으로 끝내라고 하면 그건 그냥 못 하는 일이 된다. 그래서 세는 값을
`state` 가 아니라 save 에 둔다(`nodsGiven`·`notesLeft`) — `state` 에 두면 죽는 순간
사라진다.

그리고 **「불을 쬐지 않고」가 실제로 달성 가능해졌다.** 예전에는 모닥불에 나가는 길이
없어서, 모르고 한 칸 잘못 밟으면 그 판이 그 자리에서 끝났다 (위의 모닥불 절 참고).
...
- **층 성격 8종** — 층마다 하나씩 붙는다. 진입 문구가 그것을 흘리고, 층수 옆에 이름이 남는다 (두드리면 뜻을 말한다)
- **첫 판 안내 4줄** — 튜토리얼 화면이 아니라 상황이 왔을 때 로그에 한 줄씩. 두 번째 판부터는 안 나온다
...
- 업적 26종, 달성 시 구석 알림 — 일곱은 「어떻게 올랐는가」에 붙고(모닥불·거래·벼림·물약 없이 · 손으로만 · 닿지 않고 · 다섯 사람 모두로), 둘은 판을 넘어 쌓인다(끄덕임 · 쪽지)
...
이 저장소는 **3000 번을 쓰지 않는다.** 세상에서 제일 붐비는 포트라,
거기 정적 서버를 띄워 두면 옆 프로젝트를 올리려다 막힌다 — 그런데 그 서버는
검사가 끝나도 떠 있어서, 며칠 뒤에 「이거 왜 안 되지」가 된다.

붙박이 정적 서버는 **8123**, 스스로 띄우는 검사들은 **812x–815x**,
파티 서버는 **8787·8788** 이다. 새 도구를 만들 때도 그 범위에서 고른다.
