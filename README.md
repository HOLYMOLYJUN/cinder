...
**시작 무기는 그 갈래에서 제일 약한 것이어야 한다.** 마법사에게 나무 지팡이(주문 9)를
쥐여 줬더니 1층부터 주문 17 로 출발했다 — 기사가 12 로 시작하는데 마법은 마방을
때리므로 더 세다. 그래서 `낡은 지팡이`(주문 7)를 따로 만들었다.
...
> 세 번을 고쳤다. 높이를 맞추고(①), 바닥에 붙이고(②), 떨림을 걷어냈다(③).
> ②가 ③을 불렀고 ①이 ②를 불렀다. **화면 크기가 실시간으로 변하는 것을 다룰 때는
