...
처음에는 60판으로 재고 「클리어율이 기사 57→73% 로 뛰었다」고 적었다. **틀렸다.**
비교 대상이 README 에 적혀 있던 오래된 숫자였는데, 그 사이 게임이 이미 위로 흘러 있었다.
`git worktree` 로 **바로 전 커밋을 실제로 돌려서** 기준선을 다시 잡으니 기사 60% ·
마법사 91.7% · 드워프 71.7% · 엘프 83.3% 였다 — 내가 만진 것과 무관하게 이미 그랬다.

그리고 60판은 표준오차가 6%p 쯤이라, 같은 코드로 두 번 돌려도 열몇 %p 가 움직인다.
실제로 창병 셋 값 하나(+4→+3)만 고치고 다시 쟀더니 기사가 56.7 → 73.3% 로 나왔다 —
그럴 리 없는 변화다. **200판 아래로는 튜닝 근거로 쓰지 않는다.**

숫자를 고칠 때 기준선은 기억이나 문서가 아니라 **그때 그 코드를 돌려서** 잡을 것.
