...
function rangedAttack(dir) {
  ...
  /* 이번 턴에 하나 줄므로 3 은 "두 턴 막힌다", 4 는 "세 턴 막힌다".

     2·3 이었다. 원거리 둘과 근접 셋의 간격이 너무 벌어졌는데(200판 클리어
     57·61% 대 18~25%), 사거리도 위력도 지팡이 수치도 거의 안 움직였다.
     수치를 보면 원거리 둘은 처치가 43~44, 근접 셋은 23~31 이다 —
     **다가오는 동안 공짜로 때리는 턴이 쌓인 것**이지 한 방이 셀 것이 아니다.
     그러면 손대야 할 것은 위력이 아니라 **간격**이다. */
  state.rangedCd = isMagicAttack(p) ? 4 : 3;
