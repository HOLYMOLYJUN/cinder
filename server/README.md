...
친구와 같은 방에서 한 줄씩 주고받는 것, 그리고 층에 남는 흔적(죽은 자리 · 벽의 쪽지).
...
## 흔적 — 죽은 자리와 벽의 쪽지 (`/marks/**`)

방과 관전은 둘 다 **동시에 두 명**이 있어야 쓸모가 생긴다. 혼자 하는 게임에서 그런 순간은
거의 안 온다. 흔적은 비동기라, 아무도 접속해 있지 않아도 남이 지나간 자리가 내 판에 남아 있다.
설정과도 맞물린다 — 이 탑은 원래 「이름을 적으면 올라갔고, 아무도 내려오지 않았다」는 곳이라
남의 시체가 층에 쌓여 있는 것이 세계관 그 자체다.

방과 달리 여기는 **소켓이 아니라 그냥 HTTP** 다. 실시간일 이유가 없고, 층에 들어설 때 한 번
...
간다.** 그래서 검열할 것이 아예 없다. 문구는 `js/marks.js` 에서만 만들어지므로 늘리거나
다른 말로 옮길 때도 서버는 건드리지 않는다.

지키는 것:

- 한 층에 40개까지. 넘으면 **끄덕인 수 → 최근 순**으로 자른다. 오래된 것이 아니라
  **아무도 안 읽은 것**이 먼저 밀려나야 층이 쓸모 있는 자리로 남는다
- 기본 수명 14일. 끄덕일 때마다 늘어난다(최대 5배) — 좋은 자리는 오래 간다
- 한 uid 당 3초에 하나
- 층·좌표·번호는 정수로 자르고 이름은 16자에서 끊는다
